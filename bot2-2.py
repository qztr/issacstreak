...
PATH_TO_CSV = "/root/isaac-streak/flask"
...
class Bot(commands.Bot):
    ...
    def __init__(self):
        super().__init__(token=f'{SECRET_OAUTH}', prefix=['BibleThump '], initial_channels=self.get_channels())
    ...
    async def streak(self, ctx: commands.Context, arg):
        if ctx.author.is_mod:
            try:
                ...
                auth_commands_log(f"{ctx.message.channel.name} , streak:{arg} , {r.text}")

                if r.text == "success":
                    await ctx.send(f'Streak updated successfully, {ctx.author.name}! Leaderboard on isaac-streak.fun')
            except HTTPError as e:
                print(e.code)
            time.sleep(15)

        else:
            await ctx.send(f'Only mod can update TearGlove. Leaderboard on isaac-streak.fun')
            time.sleep(15)
    ...
    async def best(self, ctx: commands.Context, arg):
        if ctx.author.is_mod:
            try:
                ...
                if r.text == "success":
                    await ctx.send(f'Personal best streak updated successfully, {ctx.author.name}! Leaderboard on isaac-streak.fun')
            except HTTPError as e:
                print(e.code)
            time.sleep(15)
    ...
    async def category(self, ctx: commands.Context, arg):
        if ctx.author.is_mod:
            try:
                ...
                auth_commands_log(f"{ctx.message.channel.name} , category:{arg} , {r.text}")
                if r.text == "success":
                    await ctx.send(f'Category updated successfully, {ctx.author.name}! Leaderboard on isaac-streak.fun')
            except HTTPError as e:
                print(e.code)
            time.sleep(15)
        else:
...
def reload_bot():
    threading.Timer(300, restart).start() # время, через которое запускать функцию - в секундах.
